...
}(this, () =>
{
  ...
  function setupPDF()
  {
    ...
    toArray(dom.wrapper.querySelectorAll(SLIDES_SELECTOR))
      .forEach((slide) =>
      {
        ...
        if (slide.classList.contains('stack') === false)
        {
          ...
          const contentHeight = getAbsoluteHeight(slide);
          const numberOfPages = Math.max(Math.ceil(contentHeight / pageHeight), 1);
    ...
    // Show all fragments
    toArray(dom.wrapper.querySelectorAll(`${SLIDES_SELECTOR} .fragment`))
      .forEach((fragment) =>
      {
  ...
  function createBackground(slide, container)
  {
    ...
    const computedBackgroundColor = window.getComputedStyle(element)
      .backgroundColor;
    if (computedBackgroundColor)
    {
      const rgb = colorToRgb(computedBackgroundColor);
  ...
  {
    const numberOfSlides = dom.wrapper.querySelectorAll(SLIDES_SELECTOR)
      .length;
  ...
  function showPreview(url)
  {
    ...
    dom.overlay.querySelector('iframe')
      .addEventListener('load', (event) =>
      {
        dom.overlay.classList.add('loaded');
      }, false);

    dom.overlay.querySelector('.close')
  ...
  function updateBackground(includeAll)
  {
    ...
    {
      const previousVideo = previousBackground.querySelector('video');
    ...
    if (currentBackground)
    {
      ...
      if (/\.gif/i.test(backgroundImageURL))
      {
        ...
        window.getComputedStyle(currentBackground)
          .opacity;
  ...
  function showSlide(slide)
  {
    ...
    toArray(slide.querySelectorAll('video, audio'))
      .forEach((media) =>
      {
        let sources = 0;
  ...
  function formatEmbeddedContent()
  {
    ...
    {
      toArray(dom.slides.querySelectorAll(`iframe[${sourceAttribute}*="${sourceURL}"]`))
        .forEach((el) =>
        {
  ...
  function startEmbeddedContent(slide)
  {
    ...
    {
      // Restart GIFs
      toArray(slide.querySelectorAll('img[src$=".gif"]'))
        .forEach((el) =>
        {
      ...
      toArray(slide.querySelectorAll('iframe[data-src]'))
        .forEach((el) =>
        {
  ...
  function stopEmbeddedContent(slide)
  {
    if (slide && slide.parentNode)
    {
      ...
      toArray(slide.querySelectorAll('iframe'))
        .forEach((el) =>
        {
      ...
      // YouTube postMessage API
      toArray(slide.querySelectorAll('iframe[src*="youtube.com/embed/"]'))
        .forEach((el) =>
        {
      ...
      // Vimeo postMessage API
      toArray(slide.querySelectorAll('iframe[src*="player.vimeo.com/"]'))
        .forEach((el) =>
        {
          if (!el.hasAttribute('data-ignore') && typeof el.contentWindow.postMessage === 'function')
          {
  ...
  function getIndices(slide)
  {
    ...
    if (slide)
    {
      ...
      if (isVertical)
      {
        v = Math.max(toArray(slide.parentNode.querySelectorAll('section'))
          .indexOf(slide), 0);
      }
    }
    ...
    {
      const hasFragments = currentSlide.querySelectorAll('.fragment')
        .length > 0;
      if (hasFragments)
      {
        ...
        {
          f = currentSlide.querySelectorAll('.fragment.visible')
            .length - 1;
        }
      }
    }
  ...
  function getSlideBackground(x, y)
  {
    ...
    const horizontalBackground = dom.wrapper.querySelectorAll('.backgrounds>.slide-background')[x];
    const verticalBackgrounds = horizontalBackground && horizontalBackground.querySelectorAll('.slide-background');
  ...
  function navigateFragment(index, offset)
  {
    if (currentSlide && config.fragments)
    {
      ...
      if (fragments.length)
      {
        ...
        {
          const lastVisibleFragment = sortFragments(currentSlide.querySelectorAll('.fragment.visible'))
            .pop();
  ...
  function cueAutoSlide()
  {
    ...
    if (currentSlide)
    {
      ...
      const fragmentAutoSlide = currentFragment ? currentFragment.getAttribute('data-autoslide') : null;
      ...
      if (currentSlide.querySelectorAll('.fragment')
        .length === 0)
      {
      ...
      if (autoSlide && !autoSlidePaused && !isPaused() && !isOverview() && (!Reveal.isLastSlide() || availableFragments()
          .next || config.loop === true))
      {
  ...
  function navigateLeft()
  {
    ...
    if (config.rtl)
    {
      if ((isOverview() || nextFragment() === false) && availableRoutes()
        .left)
      {
        slide(indexh + 1);
      }
    ...
    // Normal navigation
    else if ((isOverview() || previousFragment() === false) && availableRoutes()
      .left)
    {
      slide(indexh - 1);
    }
  ...
  function navigateRight()
  {
    ...
    if (config.rtl)
    {
      if ((isOverview() || previousFragment() === false) && availableRoutes()
        .right)
      {
        slide(indexh - 1);
      }
    ...
    // Normal navigation
    else if ((isOverview() || nextFragment() === false) && availableRoutes()
      .right)
    {
      slide(indexh + 1);
    }
  ...
  function navigatePrev()
  {
    ...
    if (previousFragment() === false)
    {
      ...
      else
      {
        ...
        if (config.rtl)
        {
          previousSlide = toArray(dom.wrapper.querySelectorAll(`${HORIZONTAL_SLIDES_SELECTOR}.future`))
            .pop();
        }
        else
        {
          previousSlide = toArray(dom.wrapper.querySelectorAll(`${HORIZONTAL_SLIDES_SELECTOR}.past`))
            .pop();
        }

        if (previousSlide)
        {
          const v = (previousSlide.querySelectorAll('section')
            .length - 1) || undefined;
          const h = indexh - 1;
          slide(h, v);
        }
  ...
  function onDocumentKeyDown(event)
  {
    ...
    const resumeKeyCodes = [66, 190, 191];
    let key;
  ...
  function onProgressClicked(event)
  {
    ...
    const slidesTotal = toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR))
      .length;
